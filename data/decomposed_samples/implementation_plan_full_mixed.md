...
Практические файлы должны быть достаточно длинными, потому что короткие prompts плохо отражают реальный `AGENTS.md` или `CLAUDE.md`.
Сравнение должно оставаться честным: русский и английский варианты должны передавать один и тот же смысл.
Mixed sample должен быть отдельным сценарием и не должен заменять strict RU/EN comparison.
...
- `dev_prompt_ru.md` и `dev_prompt_en.md`.
- `system_prompt_ru.md` и `system_prompt_en.md`.
- `project_rules_ru.md` и `project_rules_en.md`.
- `flores_ru.md` и `flores_en.md`.
...
Например: сначала проверь `git status --short`, потом run `python scripts/count_openai_tiktoken.py`, затем compare `ru_en` and `mixed_en` ratios.
Если `mixed_en` почти равен `1.0`, не делай громкий claim; просто record the observation and keep the methodology honest.
...
Сохрани revised samples в `data/samples/`.
Сохрани updated counting script в `scripts/`.
Сохрани layer note в корне проекта или в `docs/layers/`.
Сохрани generated CSV в `results/`, но проверь его перед commit.
